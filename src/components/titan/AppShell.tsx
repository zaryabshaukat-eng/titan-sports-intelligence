...
import {
  ...
  Activity, Menu, X, Clock, Palette, PanelLeftClose, PanelLeftOpen, Keyboard,
} from "lucide-react";
...
export function AppShell({ children }: { children: ReactNode }) {
      ...
      <nav className="flex-1 space-y-4 overflow-y-auto px-2 pb-4">
        {nav.map((section) => (
          <div key={section.group}>
            {!collapsed && (
              <div className="px-3 pb-1.5 text-[10px] font-semibold uppercase tracking-[0.15em] text-muted-foreground/60">
                {section.group}
              </div>
            )}
      ...
      <div className="border-t border-white/5 p-2">
        ...
        {!collapsed && (
          <button
            ...
            className="mb-2 flex w-full items-center gap-2 rounded-lg px-3 py-2 text-xs text-muted-foreground hover:bg-white/5 hover:text-foreground transition-colors"
          >
            <Keyboard className="h-3.5 w-3.5" />
            <span>Keyboard shortcuts</span>
            <kbd className="ml-auto rounded border border-white/10 bg-white/5 px-1.5 py-0.5 text-[10px]">?</kbd>
          </button>
        )}
        <div className={`glass rounded-lg p-3 ${collapsed ? "hidden" : ""}`}>
  ...
  return (
    <div className="dark min-h-screen text-foreground">
      ...
      {/* G-chord indicator */}
      {gPressed && (
        <div className="fixed bottom-4 right-4 z-[500] flex items-center gap-2 rounded-lg border border-primary/30 bg-[oklch(0.15_0.025_260)] px-3 py-2 text-xs shadow-xl">
      ...
      <div className="flex min-h-screen w-full">
        ...
        >
          ...
          <button
            ...
            className="absolute -right-3 top-20 z-10 grid h-6 w-6 place-items-center rounded-full border border-white/10 bg-[oklch(0.18_0.025_260)] text-muted-foreground shadow-md hover:text-foreground transition-colors"
          >
        ...
        <div className="flex min-w-0 flex-1 flex-col">
          ...
          <header className="sticky top-0 z-30 flex h-14 items-center gap-3 border-b border-white/5 bg-[oklch(0.13_0.02_260)]/70 px-4 backdrop-blur-xl md:px-6">
            ...
            <div className="ml-auto flex items-center gap-2">
              ...
              <div className="hidden items-center gap-1.5 rounded-md border border-emerald/20 bg-emerald/10 px-2.5 py-1 text-xs font-medium text-emerald md:flex">
                <span className="h-1.5 w-1.5 rounded-full bg-emerald shadow-[0_0_8px] shadow-emerald animate-pulse" />
                LIVE FEED
              </div>
              ...
              <button
                ...
                className="hidden sm:grid h-9 w-9 place-items-center rounded-md border border-white/5 bg-white/5 text-muted-foreground hover:text-foreground transition-colors"
              >
          ...
          <main className="min-w-0 flex-1 p-4 md:p-6 lg:p-8 transition-all duration-200">{children}</main>
        </div>
      </div>
    </div>
  );
