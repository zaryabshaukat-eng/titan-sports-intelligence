...
import {
  ...
  Target, Activity, RefreshCw, BookOpen, StickyNote, CheckCircle2,
  AlertTriangle, Info, ArrowRight, Zap, Brain, LineChart as LineIcon,
} from "lucide-react";
import {
...
/* ──────────── mock report data ──────────── */
const REPORT_LIST = [
  { id: "r1", title: "Weekly Intelligence Digest", period: "Week 41 — Jul 2026", size: "3.2 MB", author: "Titan AI",            date: "Jul 12, 2026", status: "final" as const, sections: 8 },
...
const timelineEvents = [
  {
    time: "Mon Jul 7 · 00:01",
    type: "system",
    icon: RefreshCw,
    title: "Week 41 monitoring started",
    detail: "248 fixtures queued across 18 competitions. All 9 engines online.",
    color: "text-primary bg-primary/10 border-primary/20",
  },
  {
    time: "Mon Jul 7 · 09:14",
    type: "signal",
    icon: Target,
    title: "First high-confidence signal — Bayern AH -0.5",
    detail: "+4.9% EV at Pinnacle. Confidence 74. Statistical and Market engines aligned.",
    color: "text-emerald bg-emerald/10 border-emerald/20",
  },
  {
    time: "Tue Jul 8 · 14:30",
    type: "model",
    icon: Brain,
    title: "Tactical Engine entered training cycle",
    detail: "Formation signal weighting reduced to 60% across all affected markets. ETA: Jul 15.",
    color: "text-warning bg-warning/10 border-warning/20",
  },
  {
    time: "Thu Jul 10 · 11:22",
    type: "signal",
    icon: Zap,
    title: "Sharp steam move — Man City Over 2.5",
    detail: "Line 1.92 → 1.85. Coordinated sharp action confirmed across Pinnacle, Betfair, Bet365.",
    color: "text-emerald bg-emerald/10 border-emerald/20",
  },
  {
    time: "Fri Jul 11 · 16:05",
    type: "arbitrage",
    icon: LineIcon,
    title: "Arbitrage window opened — 3-book opportunity",
    detail: "Real Madrid vs Barcelona. ROI 3.2%. Window closed within 8 minutes.",
    color: "text-primary bg-primary/10 border-primary/20",
  },
  {
    time: "Sat Jul 12 · 20:00",
    type: "result",
    icon: CheckCircle2,
    title: "Man City vs Arsenal settled — Over 2.5 ✓",
    detail: "Final score 3–1. CLV +2.3%. Consensus model validated.",
    color: "text-emerald bg-emerald/10 border-emerald/20",
  },
  {
    time: "Sun Jul 13 · 23:00",
    type: "system",
    icon: RefreshCw,
    title: "Week 41 monitoring closed",
    detail: "63 value signals · 9 arbitrage opportunities · 87.4% ensemble health · £24.7K EV generated.",
    color: "text-primary bg-primary/10 border-primary/20",
  },
];
...
const analystNotes = [
  {
    id: "an1",
    author: "Titan Analyst",
    date: "Jul 12, 2026 · 22:14",
    tag: "Observation",
    tagVariant: "primary" as const,
    content:
      "The Tactical Engine training cycle had a smaller-than-expected impact on signal quality this week. The Consensus Engine compensated well by upweighting the Statistical and Historical inputs. Worth monitoring whether this reweighting holds post-training.",
  },
  {
    id: "an2",
    author: "Titan Analyst",
    date: "Jul 11, 2026 · 10:45",
    tag: "Risk Flag",
    tagVariant: "warning" as const,
    content:
      "Steam move on Man City Over 2.5 was unusually fast — line moved 7 ticks in under 3 minutes. Suggests high-volume sharp syndicate activity. Cross-check with bookmaker suspension logs for next week's reporting cycle.",
  },
  {
    id: "an3",
    author: "Titan Analyst",
    date: "Jul 8, 2026 · 09:30",
    tag: "Model Note",
    tagVariant: "emerald" as const,
    content:
      "Closing-line value averaged +2.7% this week across settled bets, confirming the model consistently identifies edges that persist to market close. This is the third consecutive week above +2.5% CLV.",
  },
];
...
const SECTIONS = [
  ...
  { id: "s7", title: "Week Timeline",            icon: Clock       },
  { id: "s8", title: "Analyst Notes",            icon: StickyNote  },
];
...
function ReportViewer({ reportId, onClose }: { reportId: string; onClose: () => void }) {
  ...
  const handleExport = () => {
    const data = JSON.stringify(
      { report: report.title, generated: new Date().toISOString(), sections: SECTIONS.map((s) => s.title) },
      null, 2,
    );
    ...
    <div className={`flex flex-col ${fullscreen ? "fixed inset-0 z-[400] bg-[oklch(0.14_0.02_260)]" : ""}`}>
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto px-6 py-8 max-w-4xl mx-auto w-full print:px-0">
                      ...
                      <td className="px-4 py-3 w-40">
                        <div className="flex items-center gap-2">
                          <div className="h-1 flex-1 overflow-hidden rounded-full bg-white/5">
                            <div
                              className={`h-full rounded-full ${e.health >= 90 ? "bg-emerald" : e.health >= 75 ? "bg-primary" : "bg-warning"}`}
          ...
          {/* S7 — Week Timeline */}
          <div id="s7" ref={(el) => { sectionRefs.current["s7"] = el; }} className="mb-10 scroll-mt-4">
            <div className="text-[10px] font-semibold uppercase tracking-[0.2em] text-primary mb-2">07 — Week Timeline</div>
            <h2 className="font-display text-xl font-bold mb-4">Key Events — Week 41</h2>
                ...
                {timelineEvents.map((event, i) => {
                  ...
                  return (
                    <div key={i} className="relative flex gap-4">
                      {/* Dot */}
                      <div className={`absolute -left-3.5 grid h-7 w-7 shrink-0 place-items-center rounded-full border ${event.color}`}>
                        <Icon className="h-3.5 w-3.5" />
                      </div>
                      {/* Card */}
                      <div className="ml-4 flex-1 rounded-xl border border-white/5 bg-white/[0.02] p-4 transition-all hover:border-white/10">
                        <div className="flex items-start justify-between gap-3 mb-1.5">
                          <h3 className="font-display text-sm font-semibold">{event.title}</h3>
          ...
          {/* S8 — Analyst Notes */}
          <div id="s8" ref={(el) => { sectionRefs.current["s8"] = el; }} className="mb-10 scroll-mt-4">
            <div className="text-[10px] font-semibold uppercase tracking-[0.2em] text-primary mb-2">08 — Analyst Notes</div>
            <h2 className="font-display text-xl font-bold mb-4">Annotated Observations</h2>
            <div className="space-y-4">
                ...
                <GlassCard key={note.id} className="p-5">
                  <div className="flex items-start justify-between gap-3 mb-3">
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        <div className="text-xs font-semibold">{note.author}</div>
                        <div className="text-[10px] text-muted-foreground">{note.date}</div>
                      </div>
                    </div>
                    <Badge variant={note.tagVariant} size="sm">{note.tag}</Badge>
                  </div>
              ...
              {/* Add note CTA */}
              <button className="flex w-full items-center justify-center gap-2 rounded-xl border border-dashed border-white/10 py-4 text-xs text-muted-foreground hover:border-white/20 hover:text-foreground transition-colors">
