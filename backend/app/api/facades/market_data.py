...
class MarketDataApiFacade:
    ...
    async def get_bookmaker(self, bookmaker_id: UUID) -> Bookmaker | None:
        return await BookmakerRepository(self._session).get(bookmaker_id)

    async def list_market_types(self, pagination: PaginationParams) -> PageResult[MarketType]:
        return await MarketTypeRepository(self._session).list(pagination)

    async def list_market_statuses(self, pagination: PaginationParams) -> PageResult[MarketStatus]:
