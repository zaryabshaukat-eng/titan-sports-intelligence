...
from app.api.facades.infrastructure import InfrastructureApiFacade
...
async def health(request: Request, p: R):
    _ = p
    return await InfrastructureApiFacade(request.app.state.redis).health()
...
async def status(p: R):
    _ = p
    return InfrastructureApiFacade.status()
