...
async def scenarios(p: R) -> list[ScenarioMetadataRead]:
    ...
    return [
        ...
        for x in ScenarioRegistry().metadata()
    ]
...
async def create(body: BacktestRunCreate, s: S, p: W) -> BacktestRunRead:
    _ = p
    try:
        return BacktestRunRead.model_validate(await BacktestService(s).create(body))
...
async def list_backtests(pagination: P, s: S, p: R) -> Page[BacktestRunRead]:
    _ = p
    items, total = await EvaluationRepository(s).list_runs(pagination)
    return Page(
...
async def backtest(id: UUID, s: S, p: R) -> BacktestRunRead:
    _ = p
    item = await EvaluationRepository(s).run(id)
    if item is None:
...
async def results(id: UUID, s: S, p: R) -> list[BacktestResultRead]:
    _ = p
    return [BacktestResultRead.model_validate(x) for x in await EvaluationRepository(s).results(id)]
...
async def metrics(id: UUID, s: S, p: R) -> BacktestMetricRead | None:
    _ = p
    x = await EvaluationRepository(s).metric(id)
...
async def comparisons(
    ...
    _ = p
    values = await EvaluationRepository(s).comparison_metrics([baseline_run_id, candidate_run_id])
...
async def lineage(id: UUID, s: S, p: R) -> BacktestLineageRead | None:
    _ = p
    x = await EvaluationRepository(s).lineage(id)
...
async def validation(id: UUID, s: S, p: R) -> list[BacktestValidationRead]:
    ...
    return [
        ...
        for x in await EvaluationRepository(s).validation(id)
    ]
