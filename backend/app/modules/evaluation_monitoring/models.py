...
class EvaluationRun(IdCreated, Base):
    ...
    __table_args__ = (
        UniqueConstraint("run_code", name="uq_monitoring_run_code"),
        UniqueConstraint("idempotency_key", name="uq_monitoring_run_key"),
        Index("ix_monitoring_runs_backtest_created", "backtest_run_id", "created_at"),
    )
    run_code: Mapped[str] = mapped_column(String(96), nullable=False)
    configuration_id: Mapped[UUID] = mapped_column(
        PGUUID(as_uuid=True),
        ForeignKey("monitoring_evaluation_configurations.id", ondelete="RESTRICT"),
        nullable=False,
        index=True,
    )
    ...
    status: Mapped[MonitoringStatus] = mapped_column(
        SqlEnum(MonitoringStatus, name="monitoring_status"), nullable=False
    )
...
class DriftMeasurement(RunArtifact):
    ...
    baseline_run_id: Mapped[UUID | None] = mapped_column(
        PGUUID(as_uuid=True), ForeignKey("monitoring_evaluation_runs.id", ondelete="RESTRICT")
    )
...
class Alert(RunArtifact):
    ...
    severity: Mapped[AlertSeverity] = mapped_column(
        SqlEnum(AlertSeverity, name="monitoring_alert_severity"), nullable=False
    )
...
class ValidationRecord(RunArtifact):
    ...
    status: Mapped[ValidationStatus] = mapped_column(
        SqlEnum(ValidationStatus, name="monitoring_validation_status"), nullable=False
    )
    message: Mapped[str] = mapped_column(Text, nullable=False)
